'use client';
/**
 * School Chat — Wellfound-inspired MUI chat interface.
 * GUARDRAIL: ALL state logic, polling, API calls, DatePicker preserved exactly.
 */
...
export default function SchoolChatPage() {
  ...
  const needsDate = (code) => ['INTERVIEW_SCHEDULED', 'INTERVIEW_RESCHEDULED'].includes(code);
      ...
      <Box sx={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <Box sx={{ width: { xs: '100%', md: 320 }, borderRight: '1px solid', borderColor: 'divider', bgcolor: 'white', display: { xs: activeThread ? 'none' : 'flex', md: 'flex' }, flexDirection: 'column' }}>
          <Box sx={{ p: 2, borderBottom: '1px solid', borderColor: 'divider' }}>
            <Typography variant="subtitle2" sx={{ fontWeight: 700, color: 'text.secondary', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Candidates</Typography>
          </Box>
          <Box sx={{ flex: 1, overflowY: 'auto' }}>
            {loadingThreads ? (
              ...
              <Box sx={{ p: 4, textAlign: 'center' }}>
                <Typography variant="body2" sx={{ color: 'text.secondary' }}>No conversations yet. Threads are created when candidates enter your pipeline.</Typography>
              </Box>
            ) : (
              <List disablePadding>
                {threads.map((t) => (
                  ...
                  >
                    <ListItemAvatar>
                      <Avatar sx={{ bgcolor: '#DBEAFE', color: '#2563EB', fontWeight: 700, fontSize: '1rem' }}>
                        {getInitial(getDisplayName(t))}
                      </Avatar>
                    </ListItemAvatar>
                    <ListItemText
                      primary={
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <Typography variant="subtitle2" sx={{ fontWeight: 600, color: 'text.primary', noWrap: true }}>{getDisplayName(t)}</Typography>
        ...
        <Box sx={{ flex: 1, display: { xs: activeThread ? 'flex' : 'none', md: 'flex' }, flexDirection: 'column', bgcolor: '#F8FAFC' }}>
          {!activeThread ? (
            <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', p: 4, textAlign: 'center' }}>
              ...
              <Typography variant="h6" sx={{ fontWeight: 700, color: 'text.primary', mb: 1 }}>Select a candidate</Typography>
              <Typography variant="body2" sx={{ color: 'text.secondary', maxWidth: 300 }}>Choose a candidate from the left to view activity log and send actions.</Typography>
            </Box>
          ) : (
            <>
              {/* Chat Header */}
              <Box sx={{ p: 2, borderBottom: '1px solid', borderColor: 'divider', bgcolor: 'white', display: 'flex', alignItems: 'center', gap: 2 }}>
                ...
                <Avatar sx={{ bgcolor: '#DBEAFE', color: '#2563EB', fontWeight: 700 }}>{getInitial(getDisplayName(activeThread))}</Avatar>
                <Box>
                  <Typography variant="subtitle1" sx={{ fontWeight: 700, color: 'text.primary', lineHeight: 1.2 }}>{getDisplayName(activeThread)}</Typography>
              ...
              <Box sx={{ flex: 1, overflowY: 'auto', p: 3, display: 'flex', flexDirection: 'column', gap: 2 }}>
                {loadingMessages ? (
                  ...
                  <Typography variant="body2" sx={{ color: 'text.tertiary', textAlign: 'center', mt: 4 }}>No activity yet. Use the action buttons below to communicate.</Typography>
                ) : (
                  messages.map((msg) => {
                  ...
                  {showDatePicker && (
                    <Box sx={{ display: 'flex', gap: 1, mb: 1.5, alignItems: 'center' }}>
                      <input
                        ...
                        style={{ padding: '6px 12px', borderRadius: '8px', border: '1px solid #E2E8F0', fontSize: '0.875rem', fontFamily: 'inherit' }}
                      />
                      <Button variant="contained" size="small" onClick={() => handleDateAction(showDatePicker)} disabled={!selectedDate} sx={{ borderRadius: '8px', fontWeight: 600 }}>
                        Confirm
                      </Button>
                      <Button variant="outlined" size="small" onClick={() => { setShowDatePicker(null); setSelectedDate(''); }} sx={{ borderColor: 'divider', color: 'text.secondary', borderRadius: '8px', fontWeight: 600 }}>
                        Cancel
                      </Button>
                    </Box>
                  )}
